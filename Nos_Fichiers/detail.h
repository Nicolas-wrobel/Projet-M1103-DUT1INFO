 ...
 */
typedef std::vector <std::string> CVLine;
...
struct CMyParam {
    ...
    std::map <std::string, std::string> MapParamChar;
    /** List of parameters of type unsigned*/
    std::map <std::string, int> MapParamUnsigned;
 ...
 */
struct AuthorizedKey2 {
    /** List of authorized key for the type char in a struct CMyParam*/
    const std::vector <std::string> VParamChar {"KeyUp", "KeyDown", "KeyLeft", "KeyRight", "TokenP1", "TokenP2", "Teleport", "Bounty"};
                                                //"Candy","Ghost","Snack","PacPlayer"};
 ...
 */
const AuthorizedKey2 KAuthorizedKey2;
